...
def registerme(update: Update, context: CallbackContext):
    if update.message.chat.type == "private":
        ...
        if db_commands.insertRecordTelegramUser(update.message.from_user.id,str(update.message.from_user.first_name)+" "+str(update.message.from_user.last_name)):
            bot.send_message(
                chat_id=update.message.chat_id,
                text=f'Hi 😎 {str(update.message.from_user.first_name)+" "+str(update.message.from_user.last_name)}. You have successfully registered 🎉🎉'
            )
            bot.send_message(
                chat_id=update.message.chat_id,
                text='Wish You 🎅🍰 Merry Christmas Day 🍰🎅'
